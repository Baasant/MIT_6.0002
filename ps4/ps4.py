...
def calc_pop_avg(populations, n):
    ...
    result=float(totalpop/len(populations)  )
    return result
    #pass  # TODO
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        Returns:
            int: The total bacteria population at the end of the update
        """
        surviving_bacteria=[] #make a list of surviving_bacteria
        for ind, bac in enumerate(self.bacteria):
            if bac.is_killed(): # check the bac
                self.bacteria.pop(ind) #if the bacteria die pop it from it (remove it )
            else: #if bacteria alive check its resistance
                if self.get_antibiotics() and not bac.get_resistant():# if i give the pation antibiotics and bac has no resistant (die) remove from the list
                    self.bacteria.pop(ind)
                else:
                    popDensity = self.get_total_pop() / float(self.max_pop)
                    try:
                        surviving_bacteria.append(bac.reproduce(popDensity))
                    except NoChildException:
                        continue
        self.bacteria= self.bacteria + surviving_bacteria
        return self.get_total_pop()
