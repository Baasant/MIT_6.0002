...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles={} #dictionary to write the amount of dist in (x,y)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        x, y = map(math.floor, [pos.get_x(), pos.get_y()])#used map (math.floor,x,y)to get position and convert it to float
        capacity=max(0,self.tiles[x][y] - capacity)#after clean this pos change the capicity
        raise NotImplementedError
    ...
    def is_position_in_room(self, pos):
        ...
        """
        if (math.floor(pos.x), math.floor(pos.y)) in self.tiles.keys():
            return True
        else:
            return False
        #raise NotImplementedError
    ...
    def get_dirt_amount(self, m, n):
        ...
        Returns: an integer
        """
        return self.tiles[(m,n)] # return amount of dist by its key
        #raise NotImplementedError
    ...
    for t in range(num_trials):
        ...
        while float((room.get_num_cleaned_tiles() / room.get_num_tiles())) <= min_coverage:
            for robo in robot_list:
                robo.update_position_and_clean()
            step=step+1
        trial_list.append(step)
    mean=float(sum(trial_list)) / len(trial_list)
    return mean
